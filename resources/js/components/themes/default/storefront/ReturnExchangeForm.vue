    ...
    <div>
        ...
        <div class="flex min-h-full flex-1 flex-col justify-center px-6 py-12 lg:px-8">
            <div class="sm:mx-auto sm:w-full sm:max-w-sm">
                <h2 class="mb-3 text-center text-2xl/9 font-bold tracking-tight text-gray-900">{{ $t('Returns & Exchanges') }}</h2>
                <p>{{ $t('To make a return or exchange, fill out the form below. We will process your request as quickly as possible. If we need more info to complete the request, we will contact you.') }}</p>
            </div>
            <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
                <form @submit.prevent="submitForm" class="space-y-6">
					...
					<button type="submit" class="w-full flex justify-center items-center rounded-lg bg-gray-800 px-4 py-3 text-sm font-semibold text-white shadow-sm hover:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 transition-colors duration-200 cursor-pointer mb-2">{{ $t('Next') }}</button>
                </form>
            </div>
        </div>
    </div>
