...
onMounted(async () => {
    ...
    await listingStore.fetchProductsByKeyword(route.query.keyword, getUrlParams());
...
onBeforeRouteUpdate(async (to, from, next) => {
    ...
    await listingStore.fetchProductsByKeyword(to.query.keyword, {  ...to.query, sort: sortBy.value });
...
<template>
    <div class="container mx-auto px-4">
		<section class="py-6">
			<h1 class="text-3xl font-bold text-gray-700">{{ route.query.keyword }}</h1>
		</section>
		...
		<section v-if="loaded" class="grid grid-cols-12 gap-4">
			...
			<div class="col-span-12 lg:col-span-10">
				<template v-if="+listingStore.paginationInfo.total > 0">
					...
					<div class="lg:flex justify-center items-center space-x-2 mt-10">
						...
						>
							<LocalizedLink 
								:to="{ 
									path: `/search-result?keyword=${$route.params.keyword}`, 
				...
				<div v-else class="text-center pt-10">
					<p class="text-gray-600">{{ $t('There is no product in this search.') }}</p>
					<a
						:href="`/search-result?${$route.query.keyword}`"
