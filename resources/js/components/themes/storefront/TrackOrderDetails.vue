    ...
    <div class="row justify-content-center">
        <div class="col-12 col-lg-7">
            <div v-if="loaded === true" class="card card-body">
                ...
                <p v-if="Object.keys(orderTrackingDetails).length > 0" class="fw-bold alert alert-info">
                ...
                <table class="table table-striped hover" v-if="order.comments.length > 0">
                    <thead>
                        <tr>
                            <th>{{$t('Date')}}</th>
                            <th>{{$t('Status')}}</th>
...
export default {
    ...
    created() {
        ...
        this.$store.dispatch('orderDetailsByRef', this.$route.params.ref).then(() => {
            this.$store.dispatch('orderTrackingDetails', this.order.id)
        }).catch(error => {
