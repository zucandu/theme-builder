<template>
    <form @submit.prevent="subscribeNewsletter" class="w-100">
        <div class="input-group py-1 border bg-white rounded">
            ...
            <input v-model="formdata.email" type="email" class="form-control bg-transparent border-0" required :placeholder="$t('Your email address')">
            <span class="input-group-text bg-transparent border-0 p-0 pe-1">
                <button class="btn btn-success" type="submit">{{ $t('Subscribe') }}</button>
            </span>
        </div>
        <!-- <div class="d-flex">
            <input v-model="formdata.email" type="text" class="form-control z-subscribe-input me-2" required>
