<template>
    <nav :class="`navbar-${menuKey} navbar navbar-expand-md navbar-light`"> 
        <div id="offcanvas-menu" class="collapse navbar-collapse offcanvas offcanvas-start">
            <template v-if="loaded">
                ...
                <button id="closeOffcanvas" type="button" class="btn-close d-none" aria-label="Close"></button>
            </template>
    ...
    watch: {
        offcanvasStatus(v) {
            if(v) {
                ...
                this.$emit('updateOffcanvasStatus', v)
            }
        }
    }
}
